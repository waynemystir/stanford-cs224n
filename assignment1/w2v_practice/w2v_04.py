...
def sgd(f,x0,itrs,learning_rate,print_every=1000):
    x,expcost,st = x0,None,time.time()
    for i in range(itrs):
        cost,grad=f(x)
        x -= learning_rate*grad
        expcost = cost if expcost is None else 0.9*expcost + 0.1*cost
        if i % print_every == 0:
            print("SGD iter(%d) expcost(%f) cost(%f) in (%f) seconds" % (i,expcost,cost,time.time()-st))
            st=time.time()
    return x
...
def run():
    ...
    st = time.time()
    vectors = sgd(lambda vecs: sgd_wrapper(tokens_encoded,vecs,5,dataset,w2vModel=skipgram,w2vCAG=negSamplingCAG), vectors, 25001, 3e-1)
